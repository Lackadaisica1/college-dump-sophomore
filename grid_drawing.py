...
def print_interpreted(grid_of_stuff):
    ...
    for i in range(len(grid_of_stuff)-1, -1, -1):
        for j in range(len(grid_of_stuff)):
            print(grid_of_stuff[j][i], end = "")
        print()
...
def hollow_rectangle(color, x1, y1, x2, y2, grid):
    ...
    if x1 <= x2:
        if y1 <= y2:
           grid = horiz_line(color, x1+1, x2+1, y1, y1, grid)
           grid = horiz_line(color, x1+1, x2+1, y2, y2, grid)
           grid = vert_line(color, x1, x1, y1+1, y2+1, grid)
           grid = vert_line(color, x2, x2, y1+1, y2+1, grid)
           return grid
        else:
