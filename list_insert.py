...
def sorted_list_insert(old_head, new_node):
    ...
    if new_node.val[1] < old_head.val[1]:
        ...
        # shape of the list after this is done? this "caps" the linked list with
	# the old head of the previous linked list
    ...
    # is it using <= in the second test? this checks the first entries in
    ...
    # be true when it ends? cur is the head of the linked list, which then iterates
    # down the linked list to check the conditions enumerated above
    ...
    # to new_node? cur.next links to the new node, and the new note links 
    # backwards to cur.next (it's the penultimate node)
